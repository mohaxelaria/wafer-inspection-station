...
CKPT = Path(__file__).resolve().parent.parent / "ml" / "checkpoints" / "wafer_cnn.pt"


class CNNDetector:
    """Wraps the trained classifier behind the same interface as the heuristic."""

    def __init__(self, checkpoint: Path = CKPT):
        import numpy as np
        import torch
        ...
        self.name = f"cnn-wm811k(f1={f1:.2f})" if f1 else "cnn-wm811k"
